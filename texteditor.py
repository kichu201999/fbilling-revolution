...
def purch_create():
    ...
    def purch_sele_customer():
      ...
      global cust_tree
      cust_tree=ttk.Treeview(cuselection, height=27)
      cust_tree["columns"]=["1","2","3", "4"]
      cust_tree.column("#0", width=35)
      cust_tree.column("1", width=160)
      cust_tree.column("2", width=160)
      cust_tree.column("3", width=140)
      cust_tree.column("4", width=140)
      cust_tree.heading("#0",text="")
      cust_tree.heading("1",text="Customer/Ventor ID")
      cust_tree.heading("2",text="Customer/Ventor Name")
      cust_tree.heading("3",text="Tel.")
      cust_tree.heading("4",text="Contact Person")
      cust_tree.place(x=5, y=45)
    ...
    #   paddress.insert(0,purch_sel_combo[9])
    #   pmail.delete(0,END)
    #   pmail.insert(0,purch_sel_combo[8])
    #   psms.delete(0,END)
    #   psms.insert(0,purch_sel_combo[24])
    #   sql = "select * from company"
    #   fbcursor.execute(sql)
    ...
    # vender_combo['values'] = purch_sel_combo
  ...
  #######################END OF CREATE PORDER##############
    ...
    def purch_edit():
      ...
      def purch_sele_edit_customer():
        ...
        btn1=Button(purch_cuselection,compound = LEFT,image=tick, text="ok", width=60).place(x=15, y=610)
    ...
    class MyApp1:
      def __init__(self, parent):
        ...
        for i in porder_records:
          if True:
            purchase_tree.insert(parent='',index='end',iid=i,text='',values=('',i[39],i[2],i[3],i[25],i[5],i[6],i[7],i[8],i[9]))
          else:
            pass
        count += 1
        ...
        #   selected = purchase_tree.focus()
        #   selected_prdct= purchase_tree.item(selected)["values"][1]
        ...
        purchase_tree.bind('<Double-Button-1>' , prdctpicker)

        # fbcursor.execute('SELECT * FROM porder;')
        # pordertotalinput=0
        # j = 0
        # for i in fbcursor:
        #   purchase_tree.insert(parent='', index='end', iid=i, text='', values=(' ',i[0], i[2], i[3], i[4],i[5], i[6], i[7], i[8], i[9], i[10],i[11]))
        #   for line in purchase_tree.get_children():
        #     idsave1=purchase_tree.item(line)['values'][9]
